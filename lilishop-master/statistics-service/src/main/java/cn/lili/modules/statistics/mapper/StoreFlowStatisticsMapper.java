...
public interface StoreFlowStatisticsMapper extends BaseMapper<StoreFlow> {
     ...
     * @return 商品统计列表
     */
    @Select("SELECT goods_id,goods_name,COUNT(DISTINCT order_sn) AS orderNum,SUM(final_price) AS price,SUM(num) AS num FROM li_store_flow ${ew.customSqlSegment}")
     ...
     * @return 分类统计列表
     */
    @Select("SELECT category_id,SUM(final_price) AS price,SUM(num) AS num FROM li_store_flow ${ew.customSqlSegment}")
