...
public class AliPayPlugin implements Payment {
    ...
    public ResultMessage<Object> nativePay(HttpServletRequest request, PayParam payParam) {
        ...
        try {
            ...
            // 校验支付宝返回码：非 10000 说明下单失败（如签名错误、参数非法），返回 null 会导致前端二维码渲染崩溃
            if (response == null || !"10000".equals(response.getStr("code"))) {
                log.error("支付宝扫码下单失败：code={}, subCode={}, subMsg={}",
                        response == null ? "null" : response.getStr("code"),
                        response == null ? "null" : response.getStr("sub_code"),
                        response == null ? "null" : response.getStr("sub_msg"));
                throw new ServiceException(ResultCode.ALIPAY_EXCEPTION);
            }
            return ResultUtil.data(response.getStr("qr_code"));
        } catch (Exception e) {
            log.error("支付业务异常：", e);
            throw new ServiceException(ResultCode.PAY_ERROR);
        }
    ...
    private void checkPaymentResult(HttpServletRequest request) {
        try {
            ...
            boolean verifyResult = verifyNotifySignature(alipayPaymentSetting, map);
            if (verifyResult) {
            ...
            ThreadContextHolder.getHttpResponse().sendRedirect(domainProperties.getWap() + "/#/pay/successed");
        } catch (Exception e) {
    ...
    /**
     * 验证回调签名：普通公钥模式用 rsaCheckV1，证书模式用 rsaCertCheckV1
     */
    private boolean verifyNotifySignature(AlipayPaymentSetting alipayPaymentSetting, Map<String, String> map) throws AlipayApiException {
        if (cn.hutool.core.util.StrUtil.isNotEmpty(alipayPaymentSetting.getAlipayPublicKey())) {
            return AlipaySignature.rsaCheckV1(map, alipayPaymentSetting.getAlipayPublicKey(), "UTF-8", "RSA2");
        }
        return AlipaySignature.rsaCertCheckV1(map, alipayPaymentSetting.getAlipayPublicCertPath(), "UTF-8", "RSA2");
    }
