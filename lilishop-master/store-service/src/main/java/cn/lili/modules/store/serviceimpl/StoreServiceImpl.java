...
import cn.lili.feign.ClerkClient;
...
public class StoreServiceImpl extends ServiceImpl<StoreMapper, Store> implements StoreService {
     ...
     */
    @Autowired
    private ClerkClient clerkService;
    /**
     * 商品
     */
    @Autowired
    ...
    @Autowired
    @Lazy
    private MemberClient footprintService;
    ...
    public Store add(AdminStoreApplyDTO adminStoreApplyDTO) {
        ...
        //设置客户-店铺信息
        memberService.updateHaveShop(true, store.getId(), List.of(member.getId()));
        return store;
    ...
    private Store updateStore(StoreEditDTO storeEditDTO) {
        ...
        if (store != null) {
            ...
            //发送订单变更mq消息
            liliMessageTemplate.asyncSend(destination, store, MessageSendCallbackBuilder.commonCallback());
        }
    ...
    public boolean disable(String id) {
        ...
        if (store != null) {
            ...
            //删除店员token
            clerkService.listByStoreId(id).forEach(clerk -> {
    ...
    public List<GoodsSku> getToMemberHistory(String memberId) {
        ...
        for (FootPrint footPrint :
                footprintService.listFootprintsByStoreAndMember(currentUser.getStoreId(), memberId)) {
