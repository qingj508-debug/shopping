...
public class LogisticsServiceImpl extends ServiceImpl<LogisticsMapper, Logistics> implements LogisticsService {
    ...
    @Autowired
    private MemberClient storeLogisticsService;
    @Autowired
    ...
    public Map labelOrder(String orderSn, String logisticsId) {
        ...
        if ((LogisticsEnum.SHUNFENG.name().equals(logisticsSetting.getType()) && order.getDeliverStatus().equals(DeliverStatusEnum.DELIVERED.name()) && order.getOrderStatus().equals(OrderStatusEnum.DELIVERED.name()))
            ...
            // 店铺-物流公司设置
            StoreLogistics storeLogistics = storeLogisticsService.getOneByLogisticsAndStore(logistics.getId(), order.getStoreId());
            //获取店家信息
            ...
            //触发电子面单
            Map result = logisticsPluginFactory.filePlugin().labelOrder(labelOrderDTO);
            if (result != null && result.get("logisticCode") != null && orderService != null) {
                orderService.delivery(orderSn, result.get("logisticCode").toString(), logistics.getId());
            }
            return result;
        } else {
