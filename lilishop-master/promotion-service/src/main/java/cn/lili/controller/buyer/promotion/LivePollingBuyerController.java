...
@RestController
@Tag(name = "直播流接口")
@RequestMapping("/buyer/live/room/polling")
@RequiredArgsConstructor
public class LivePollingBuyerController {
    ...
    public ResultMessage<LivePollingDataVO> pollingData(@PathVariable("id") String id) {
        ...
        if (currentUser != null) {
            MemberCouponSearchParams param = new MemberCouponSearchParams();
            param.setMemberId(currentUser.getId());
            param.setLiveRoomId(id);
            liveCouponReceives = memberCouponService.getMemberCoupons(param);
        }
